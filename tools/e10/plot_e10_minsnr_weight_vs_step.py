"""
Usage:
python tools/e10/plot_e10_minsnr_weight_vs_step.py \
  docs/assets/e10/e10a_data/loss.jsonl \
  docs/assets/e10/e10b_data/loss.jsonl \
  --names e10a-ddpm e10b-ddim \
  --out docs/assets/e10/e10_plots/e10_minsnr_weight_mean_vs_step_ab.png
"""
...
def main():
    ...
    for path, name in zip(args.loss_jsonls, args.names):
        ...
        x, y = extract_xy(recs, "mins_snr/weight_mean")
        if not x:
            # if constant weighting, this might be absent or always 1
            continue
        plt.plot(x, y, label=name)

    plt.xlabel("step")
    plt.ylabel("mins_snr/weight_mean")
    plt.title("E10: Min-SNR weight mean vs step")
    plt.legend()
