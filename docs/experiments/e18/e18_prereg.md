# Prereg: E18 — Seed sensitivity (min-snr, cosine, NFE=25, 3 seeds)

**ID:** E18  
**Question:** How much do random seeds change under cosine (min-snr)? 
**Motivation:** Cosine previously lead to qualitivly better results both before AND in baseline config, does this uphold vs multipule seeds in min-snr as well?
...
## Execution plan
1. Run E18 with three seeds (one run per seed).
...
- If cosine a clear and stable improvement (>-3 FID), use as a part of "best min-snr config" later. 
